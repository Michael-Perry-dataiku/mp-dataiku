...
\outl0\strokewidth0 \strokec2 CREATE OR REPLACE DYNAMIC TABLE \{\{CONF\}\}.FRESHDESK_V2.TICKET_REQUESTER\
...
"tags" AS TAGS,\
TRY_PARSE_JSON(REPLACE("requester", 'None', 'null')) AS REQUESTER,\
REPLACE(REQUESTER:"email", '"','') AS REQUESTER_EMAIL,\
REPLACE(REQUESTER:"id", '"','') AS REQUESTER_REQUESTER_ID,\
REPLACE(REQUESTER:"name", '"','') AS REQUESTER_NAME,\
...
FROM RAW.FRESHDESK_DSS.TICKET_REQUESTER}